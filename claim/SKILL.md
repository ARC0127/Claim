---
name: claim
description: Coach researchers from their own scientific claims to proof obligations, or review a claim and its supporting argument. Use $claim for quantifiers, counterexamples, assumption costs, evidence applicability, and claim revisions. Coaching preserves user decisions and searches current sources for every substantive scientific change. Skill maintenance is not coaching.
---
...
- Route deliverable intent such as “完整审计”, “替我检查”, “给出 DAG/定理路线”, or “一次性完成” to the built-in review. A Theory Specification requested inside coaching uses S7 and the confirmed-record export, not a new audit.
...
Apply this contract to every formula produced through this skill, including coaching, review, `DEEP_DIVE`, `EVIDENCE_CHALLENGE`, transfer exercises, and archival documents. Brevity limits never justify leaving notation unexplained.
...
Treat every user turn as a possible change to the active scientific object. When the user adds or changes substantive scientific content, run a new `FRESH_LITERATURE_PASS` with live web search before relying on literature in the response. Apply this rule in coaching, review, `DEEP_DIVE`, `EVIDENCE_CHALLENGE`, transfer exercises, and archival work. Exporting unchanged confirmed text is not itself a scientific change; adding a new premise, source, or interpretation is.
...
1. Select one primary route based on the user's decision: coaching, review, or confirmed-record export.
...
3. Read the relevant bundled reference and follow its ownership, evidence, and output requirements.
...
7. A proof obligation is a target to prove, not evidence that a theorem exists or has been proved. Formal checking requires an actual prover result with its scope.
8. Do not create a multi-skill pipeline unless the user asks for one.
9. Apply the global formula clarity contract on every route; a formula with undefined notation is a hard output failure.
...
- “Use `$claim` to produce a complete audit of this theory claim.” -> built-in claim review.
- “Use `$claim` on these claims and Tables 1-2.” -> review the evidence-to-claim links.
